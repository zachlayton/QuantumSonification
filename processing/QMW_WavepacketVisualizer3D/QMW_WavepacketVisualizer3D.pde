...
void drawComplexWavefunctionCurve() {
  ...
  translate(width * 0.52, height * 0.55, 0);
  scale(zoom);
  rotateX(rotX);
  rotateY(rotY);

  float xScale = 4.0;
  float ampScale = 220.0;
  ...
  for (int i = 0; i < N; i++) {
    ...
    float radius = amp * ampScale;

    float y = radius * cos(ph);
    float z = radius * sin(ph);

    float hue = map(ph, -PI, PI, 0.0, 1.0);
    float bright = constrain(0.2 + amp * 3.0, 0.0, 1.0);

    stroke(hue, 0.95, bright);
    vertex(x, y, z);
  }
  endShape();

  // Draw the probability envelope as a translucent mound behind it.
  stroke(0, 0, 1, 0.35);
  strokeWeight(2);
  beginShape();
  for (int i = 0; i < N; i++) {
    float p = probabilityHistory[(writeIndex - 1 + HISTORY) % HISTORY][i];
    float x = (i - N / 2.0) * xScale;
    float y = -p * 450.0;
    float z = -180.0;
    vertex(x, y, z);
...
void drawWavefunctionWaterfall() {
  ...
  translate(width * 0.55, height * 0.58, 0);
  scale(zoom);
  rotateX(-0.95);
  rotateY(0.55);
  ...
  for (int h = 0; h < HISTORY - 1; h += tStep) {
    ...
    for (int i = 0; i < N; i += xStep) {
      drawWaterfallVertex(rowA, h, i, xScale, tScale, ampScale);
      drawWaterfallVertex(rowB, h + 1, i, xScale, tScale, ampScale);
    }

    endShape();
  }

  // Phase-colored contour lines on top, like the image.
  strokeWeight(1.25);
  noFill();

  for (int h = 0; h < HISTORY; h += 4) {
    int row = historyIndex(h);
    beginShape();
    ...
    for (int i = 0; i < N; i += xStep) {
      ...
      float hue = map(ph, -PI, PI, 0.0, 1.0);
      stroke(hue, 0.95, constrain(0.35 + p * 10.0, 0.0, 1.0), 0.9);
...
void drawWaterfallVertex(
  ...
  float ampScale
    ...
    float bright = constrain(0.18 + p * 12.0, 0.0, 1.0);
    float alpha  = constrain(0.18 + p * 2.0, 0.0, 0.75);

    fill(hue, 0.75, bright, alpha);
    vertex(x, y, z);
}
