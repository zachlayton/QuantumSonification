    ...
    def _build_ui(self) -> None:
        ...
        subtitle = tk.Label(
            ...
            text=(
                ...
                "Right-click a cell to clear it. Choose CNOT control, place it on one row; then choose CNOT target and place it on a different row in the same column."
            ),
            justify="left",
            wraplength=980,
        )
    ...
    def refresh_view(self) -> None:
        for qubit in range(self.program.n_qubits):
            for column in range(self.program.n_steps):
                ...
                visible_label = {"•": "CTRL", "⊕": "XOR"}.get(label, label)
                button.configure(text=visible_label)
