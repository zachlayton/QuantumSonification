#!/usr/bin/env python3
"""One-click Max demo host for QMW's Qiskit synthesis commands."""
...
class SynthesisMaxDemoHost:
    ...
    def _perform(self, result, revision: int, generation: int) -> None:
        ...
        for operation, frame in zip(operations, frames):
            ...
            tonnetz_nodes = project_frame_to_tonnetz(frame)[:4]
            for component_rank, node in enumerate(tonnetz_nodes):
                # The Tonnetz vertex supplies harmonic position. Its continuous
                # complex phase occupies the full 14-bit per-note bend range.
                note_pitch = 60 + node.pitch_offset
                note_velocity = int(round(127 * math.sqrt(node.probability)))
                bend_14bit = int(
                    round(8192 + (node.relative_phase / math.pi) * 8191)
                )
                mpe_channel = 2 + ((frame.index * 4 + component_rank) % 15)
                self.bridge.client.send_message(
                    "/qmw/synthesis/demo/tonnetz_note",
                    [
                        revision,
                        frame.index,
                        node.fifth_coordinate,
                        node.third_coordinate,
