/*
    Complex 16 x 16 matrix presets v3.

    Presets are targets. morphamount moves continuously from the identity
    operator toward the selected target, so the stable harmonic field is never
    discarded merely by choosing a matrix preset.
*/
autowatch = 1;
inlets = 4;
outlets = 4;
...
var imagDepth = 1.0, morphAmount = 0.5, source = "identity";
...
function emit() {
    var outputRe = new Array(CELLS);
    var outputIm = new Array(CELLS);
    var row, column, index, identityValue;
    for (row = 0; row < N; row++) {
        for (column = 0; column < N; column++) {
            index = row * N + column;
            identityValue = row === column ? 1.0 : 0.0;
            outputRe[index] = identityValue * (1.0 - morphAmount)
                + re[index] * morphAmount;
            outputIm[index] = im[index] * imagDepth * morphAmount;
        }
    }
    outlet(0, outputRe);
    outlet(1, outputIm);
    ...
    outlet(2, ["matrix", source, "morph", morphAmount,
        "imag_depth", imagDepth]);
}

function identity() {
    zeroMatrices();
    for (var k = 0; k < N; k++) re[k * N + k] = 1;
    source = "identity";
    emit();
}
...
function diagonal() {
    ...
    for (var j = 0; j < N; j++) {
        re[j * N + j] = Math.sqrt(Math.max(0, populations[j]) / peak);
    }
    source = "diagonal_populations";
    emit();
}
...
function ring() {
    ...
    source = "hermitian_coherence_ring";
    emit();
}
...
function morphamount(v) {
    morphAmount = clamp01(v);
    emit();
}
...
function cellpair(a, b) {
    ...
    b = Math.floor(num(b, 1));
    if (a < 0 || a >= N || b < 0 || b >= N || a === b) {
        outlet(2, ["error", "pair", a, b]);
        return;
    }
...
function list() {
    ...
    if (v.length !== CELLS) {
        outlet(2, ["error", inlet === 0 ? "external_re" : "external_im",
            "expected", CELLS, "received", v.length]);
        return;
    }
    target = inlet === 0 ? externalRe : externalIm;
    for (var j = 0; j < CELLS; j++) target[j] = num(v[j], 0);
    if (inlet === 0) externalReDirty = 1;
    else externalImDirty = 1;
