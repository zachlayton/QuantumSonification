...
def write_text(name, data):
    (ROOT / name).write_text(data.rstrip() + "\n")
...
def build_resonator_v4_source():
    """Bound normal quantum detuning while keeping raw gap morph explicit."""
    source = (ROOT / "qmw_density_field_harmonic_modal_resonator16_mc_v3.genexpr").read_text()
    source = source.replace(
        "QMW Density-Field Harmonic-Modal Resonator 16 MC v3",
    ...
    old = """harmonic_ratio = lane_index + 1;
...
target_ratio = mix(
    ...
    new = """harmonic_ratio = lane_index + 1;
quantum_ratio = max(lane_h, 0.01);
...
target_ratio = (lane_index == 0) + (lane_index > 0) * target_ratio;"""
    if old not in source:
        raise RuntimeError("could not locate v3 harmonic ratio block")
    return source.replace(old, new, 1)


def build_resonator_v4_gendsp():
    d = json.loads((ROOT / "qmw_density_field_harmonic_modal_resonator16_mc_v3.gendsp").read_text())
    codeboxes = [
        item["box"] for item in d["patcher"]["boxes"]
        if item["box"].get("maxclass") == "codebox"
    ]
    if len(codeboxes) != 1:
        raise RuntimeError("expected exactly one resonator codebox")
    codeboxes[0]["code"] = build_resonator_v4_source()
    return d


def build_voice_v2():
    d = build_voice()
    boxes = {x["box"]["id"]: x["box"] for x in d["patcher"]["boxes"]}
    boxes["title"]["text"] = (
        "QMW Poly Harmonic Voice 16 v2 — bounded quantum tuning"
    )
    boxes["gen"]["text"] = (
        "mc.gen~ @gen qmw_density_field_harmonic_modal_resonator16_mc_v4 @chans 16"
    )
    return d
...
def build_iq_rotator_v2_gendsp():
    """Wrap the full two-rail rotator in a loadable Gen document."""
    p = patcher((100, 100, 1220, 800))
    p["classnamespace"] = "dsp.gen"
    code = (ROOT / "qmw_analytic_iq_rotator16_v2.genexpr").read_text()
    p["boxes"] = [
        box(f"in{i}", "newobj", (30 + (i - 1) * 95, 30, 45, 22), f"in {i}",
            numinlets=0, numoutlets=1, outlettype=[""])
        for i in range(1, 6)
    ]
    p["boxes"].append(
        box("code", "codebox", (30, 85, 1130, 570), numinlets=5,
            numoutlets=7, outlettype=[""] * 7, code=code)
    )
    p["boxes"].extend([
        box(f"out{i}", "newobj", (80 + (i - 1) * 155, 700, 55, 22), f"out {i}",
            numinlets=1, numoutlets=0, outlettype=[])
        for i in range(1, 8)
    ])
    p["lines"] = [line(f"in{i}", 0, "code", i - 1) for i in range(1, 6)]
    p["lines"].extend(line("code", i - 1, f"out{i}", 0) for i in range(1, 8))
    return {"patcher": p}


def build_iq_audition_v2():
    """Expose projected audio first for compatibility, then all four IQ rails."""
    p = patcher((0, 0, 1280, 680))
    p["boxes"] = [
        box("title", "comment", (25, 18, 850, 30),
            "QMW Norm-Preserving Analytic I/Q Audition 16 v2", fontsize=18.0),
        box("desc", "comment", (25, 50, 1120, 40),
            "Both quadrature rails survive rotation. Real audio appears only at the explicit final projection; full pre/post IQ rails remain available.", linecount=2),
        *[box(name, "inlet", (35 + i * 145, 115, 30, 30)) for i, name in enumerate(
            ["pre-i", "pre-q", "post-i", "post-q", "phase", "control"])],
        box("smooth", "newobj", (615, 180, 160, 22), "mc.rampsmooth~ 2400 2400",
            numinlets=1, numoutlets=1, outlettype=["multichannelsignal"]),
        box("gen", "newobj", (220, 270, 565, 22),
            "mc.gen~ @gen qmw_analytic_iq_rotator16_v2 @chans 16", numinlets=5,
            numoutlets=7, outlettype=["multichannelsignal"] * 7),
        box("route", "newobj", (830, 180, 410, 22),
            "route phasedepth phasespread phasebias ssbhz projectionangle deltagain",
            numinlets=1, numoutlets=7, outlettype=[""] * 7),
        box("pd", "newobj", (830, 220, 125, 22), "prepend phase_depth"),
        box("ps", "newobj", (830, 250, 132, 22), "prepend phase_spread"),
        box("pb", "newobj", (830, 280, 120, 22), "prepend phase_bias"),
        box("ssb", "newobj", (830, 310, 105, 22), "prepend ssb_hz"),
        box("pa", "newobj", (830, 340, 155, 22), "prepend projection_angle"),
        box("dg", "newobj", (830, 370, 115, 22), "prepend delta_gain"),
        box("unknown", "newobj", (830, 410, 205, 22), "print qmw.iq.v2.unknown"),
        # Compatibility outlets: projected pre, projected post, delta.
        box("pre-proj", "outlet", (120, 530, 30, 30)),
        box("post-proj", "outlet", (260, 530, 30, 30)),
        box("delta", "outlet", (400, 530, 30, 30)),
        box("pre-i-out", "outlet", (560, 530, 30, 30)),
        box("pre-q-out", "outlet", (700, 530, 30, 30)),
        box("post-i-out", "outlet", (840, 530, 30, 30)),
        box("post-q-out", "outlet", (980, 530, 30, 30)),
        box("labels", "comment", (75, 575, 1030, 40),
            "PRE projection     POST projection      delta             PRE-I              PRE-Q              POST-I             POST-Q", linecount=2),
        box("load", "newobj", (25, 270, 60, 22), "loadbang"),
        box("defaults", "message", (25, 305, 170, 22),
            "phasedepth 1., phasespread 0., ssbhz 0., projectionangle 0., deltagain 2."),
    ]
    p["lines"] = [
        line("pre-i", 0, "gen", 0), line("pre-q", 0, "gen", 1),
        line("post-i", 0, "gen", 2), line("post-q", 0, "gen", 3),
        line("phase", 0, "smooth", 0), line("smooth", 0, "gen", 4),
        line("control", 0, "route", 0),
        line("route", 0, "pd", 0), line("route", 1, "ps", 0),
        line("route", 2, "pb", 0), line("route", 3, "ssb", 0),
        line("route", 4, "pa", 0), line("route", 5, "dg", 0),
        line("route", 6, "unknown", 0),
        line("pd", 0, "gen", 0), line("ps", 0, "gen", 0),
        line("pb", 0, "gen", 0), line("ssb", 0, "gen", 0),
        line("pa", 0, "gen", 0), line("dg", 0, "gen", 0),
        line("gen", 4, "pre-proj", 0), line("gen", 5, "post-proj", 0),
        line("gen", 6, "delta", 0), line("gen", 0, "pre-i-out", 0),
        line("gen", 1, "pre-q-out", 0), line("gen", 2, "post-i-out", 0),
        line("gen", 3, "post-q-out", 0),
        line("load", 0, "defaults", 0), line("defaults", 0, "route", 0),
    ]
    return {"patcher": p}
...
def build_presentation_host_v6_3():
    """Keep the harmonic anchor while layering complex and spectrum morphs."""
    d = build_presentation_host_v6_2()
    p = d["patcher"]
    boxes = {x["box"]["id"]: x["box"] for x in p["boxes"]}
    boxes["obj-1"]["text"] = "QMW POLYPHONIC COMPLEX HILBERT INSTRUMENT  v6.3"
    boxes["obj-2"]["text"] = (
        "Persistent harmonic anchor · level-compensated complex layer · independent gap-spectrum morph"
    )
    boxes["obj-17"]["text"] = (
        "mcs.poly~ qmw_poly_harmonic_voice16_v2 8 @steal 1 @parallel 1"
    )
    boxes["obj-93"]["text"] = "js qmw_harmonic_delay_list16_v2.js"
    boxes["obj-93"]["numinlets"] = 4
    boxes["obj-105"]["text"] = "js qmw_complex_matrix_presets16_v3.js"
    boxes["ui-lock"]["text"] = "Harmonic Lock (1 exact)"
    boxes["ui-lock"]["presentation_rect"] = [765, 146, 170, 20]
    boxes["ui-wet"]["text"] = "Complex Layer Morph"
    boxes["ui-wet"]["presentation_rect"] = [1128, 207, 150, 20]
    boxes["v6-wet-label"]["text"] = "Complex Layer Morph"
    boxes["v6-wet-def"]["text"] = "0.5"
    boxes["init-wet"]["text"] = "loadmess 0.5"

    # Replace the old dry/wet crossfade. The locked field now remains present:
    # anchor = 1/sqrt(1+m^2), complex = m/sqrt(1+m^2).
    p["lines"] = [item for item in p["lines"] if not (
        item["patchline"]["source"][0] == "v6-wet-trigger"
        and item["patchline"]["destination"][0] in {"v6-wet-pack", "v6-dry-sub"}
    )]
    boxes["v6-dry-sub"]["text"] = "expr 1./sqrt(1.+$f1*$f1)"

    # Make room for the new spectrum control in the harmonic body. Feedback
    # reference and scope lane move to the otherwise open safety-panel footer.
    boxes["obj-14"]["presentation_rect"] = [735, 675, 72, 22]
    boxes["obj-135"]["presentation_rect"] = [870, 675, 58, 22]
    boxes["ui-fbref"]["presentation_rect"] = [735, 652, 145, 20]
    boxes["ui-scope"]["presentation_rect"] = [865, 652, 90, 20]

    additions = [
        box("v63-spectrum-morph", "flonum", (1480, 1160, 72, 22),
            presentation=1, presentation_rect=[685, 337, 72, 22]),
        box("v63-spectrum-label", "comment", (1565, 1160, 180, 20),
            "Quantum Spectrum Morph", presentation=1,
            presentation_rect=[765, 339, 175, 20], fontsize=11.0,
            textcolor=[0.78, 0.86, 0.95, 1.0]),
        box("v63-spectrum-clip", "newobj", (1480, 1190, 68, 22), "clip 0. 1."),
        box("v63-spectrum-msg", "newobj", (1565, 1190, 205, 22),
            "prepend quantum_spectrum_morph"),
        box("v63-spectrum-init", "newobj", (1785, 1190, 82, 22),
            "loadmess 0."),
        box("v63-layer-coefficient", "newobj", (895, 955, 205, 22),
            "expr $f1/sqrt(1.+$f1*$f1)"),
        box("v63-matrix-morph-msg", "newobj", (1880, 1190, 150, 22),
            "prepend morphamount"),
    ]
    p["boxes"].extend(additions)
    p["lines"].extend([
        line("v63-spectrum-init", 0, "v63-spectrum-morph", 0),
        line("v63-spectrum-morph", 0, "v63-spectrum-clip", 0),
        line("v63-spectrum-clip", 0, "v63-spectrum-msg", 0),
        line("v63-spectrum-msg", 0, "v6-field-send", 0),
        line("v63-spectrum-clip", 0, "obj-93", 3),
        line("v6-wet-trigger", 0, "v63-layer-coefficient", 0),
        line("v63-layer-coefficient", 0, "v6-wet-pack", 0),
        line("v6-wet-trigger", 1, "v6-dry-sub", 0),
        line("v6-wet-clip", 0, "v63-matrix-morph-msg", 0),
        line("v63-matrix-morph-msg", 0, "obj-105", 0),
    ])

    # Update cached dependency names for portability outside this build folder.
    for dependency in p.get("dependency_cache", []):
        if dependency.get("name") == "qmw_poly_harmonic_voice16_v1.maxpat":
            dependency["name"] = "qmw_poly_harmonic_voice16_v2.maxpat"
        elif dependency.get("name") == "qmw_density_field_harmonic_modal_resonator16_mc_v3.gendsp":
            dependency["name"] = "qmw_density_field_harmonic_modal_resonator16_mc_v4.gendsp"
        elif dependency.get("name") == "qmw_harmonic_delay_list16_v1.js":
            dependency["name"] = "qmw_harmonic_delay_list16_v2.js"
        elif dependency.get("name") in {
            "qmw_complex_matrix_presets16_v1.js",
            "qmw_complex_matrix_presets16_v2.js",
        }:
            dependency["name"] = "qmw_complex_matrix_presets16_v3.js"
        elif dependency.get("name") == "qmw_coherence_feedback_gain16_v1.js":
            dependency["name"] = "qmw_coherence_feedback_gain16_v2.js"
        elif dependency.get("name") == "qmw_complex_matrixctrl16_v1.maxpat":
            dependency["name"] = "qmw_complex_matrixctrl16_v2.maxpat"
        elif dependency.get("name") in {
            "qmw_density_matrix_dualrail_feedback16_mc_v2.maxpat",
            "qmw_density_matrix_dualrail_feedback16_mc_v3.maxpat",
        }:
            dependency["name"] = "qmw_density_matrix_dualrail_feedback16_mc_v4.maxpat"
        elif dependency.get("name") == "qmw_density_matrix_hilbert_operator16_mc_v1.maxpat":
            dependency["name"] = "qmw_density_matrix_hilbert_operator16_mc_v2.maxpat"
    return d


def build_presentation_host_v6_4():
    """Preserve full IQ rotation and a unitary contract across preset morphs."""
    d = build_presentation_host_v6_3()
    p = d["patcher"]
    boxes = {x["box"]["id"]: x["box"] for x in p["boxes"]}
    boxes["obj-1"]["text"] = "QMW POLYPHONIC COMPLEX HILBERT INSTRUMENT  v6.4"
    boxes["obj-2"]["text"] = (
        "Full two-rail I/Q rotation · unitary preset morphs · explicit final projection · expressive feedback remains separate"
    )
    boxes["obj-105"]["text"] = "js qmw_unitary_matrix_presets16_v1.js"
    boxes["v6-audition"].update(
        text="qmw_analytic_iq_audition16_v2", numoutlets=7,
        outlettype=["multichannelsignal"] * 7,
    )
    boxes["v6-hlabel"]["text"] = "Projection Angle (rad)"
    boxes["v6-hmsg"]["text"] = "prepend projectionangle"
    boxes["v6-h-def"]["text"] = "0."
    boxes["ui-imag"]["text"] = "Unitary complex coupling"
    boxes["ui-matrix-help"]["text"] = (
        "Built-in matrices remain unitary through the entire morph. Use the feedback/editor path for intentionally nonunitary color."
    )

    # The legacy matrix editor's absolute row/column-sum safety attenuates
    # valid unitary matrices.  Disable that scaler and Hermitian editing lock
    # for the verified unitary preset route.  Feedback still has its own bound.
    additions = [
        box("v64-unitary-init", "newobj", (2050, 1190, 115, 22),
            "loadmess safety 0"),
        box("v64-hermitian-init", "newobj", (2050, 1220, 135, 22),
            "loadmess hermitian 0"),
        box("v64-contract-note", "comment", (1880, 1130, 310, 42),
            "UNITARY PRESET CONTRACT: U^H U = I. Arbitrary external matrices belong to the expressive route.", linecount=2),
    ]
    p["boxes"].extend(additions)
    p["lines"].extend([
        line("v64-unitary-init", 0, "obj-150", 0),
        line("v64-hermitian-init", 0, "obj-150", 0),
    ])

    for dependency in p.get("dependency_cache", []):
        if dependency.get("name") == "qmw_complex_matrix_presets16_v3.js":
            dependency["name"] = "qmw_unitary_matrix_presets16_v1.js"
        elif dependency.get("name") == "qmw_analytic_audition16_v1.maxpat":
            dependency["name"] = "qmw_analytic_iq_audition16_v2.maxpat"
        elif dependency.get("name") == "qmw_analytic_phase_projector16_v1.gendsp":
            dependency["name"] = "qmw_analytic_iq_rotator16_v2.gendsp"
    return d


def main():
    write_text("qmw_density_field_harmonic_modal_resonator16_mc_v4.genexpr",
               build_resonator_v4_source())
    write("qmw_density_field_harmonic_modal_resonator16_mc_v4.gendsp",
          build_resonator_v4_gendsp())
    ...
    write("qmw_analytic_iq_rotator16_v2.gendsp", build_iq_rotator_v2_gendsp())
    write("qmw_poly_harmonic_voice16_v1.maxpat", build_voice())
    write("qmw_poly_harmonic_voice16_v2.maxpat", build_voice_v2())
    write("qmw_analytic_audition16_v1.maxpat", build_audition())
    write("qmw_analytic_iq_audition16_v2.maxpat", build_iq_audition_v2())
    ...
    write("QMW_Polyphonic_Complex_Hilbert_Instrument_v6_3.maxpat", build_presentation_host_v6_3())
    write("QMW_Polyphonic_Complex_Hilbert_Instrument_v6_4.maxpat", build_presentation_host_v6_4())
