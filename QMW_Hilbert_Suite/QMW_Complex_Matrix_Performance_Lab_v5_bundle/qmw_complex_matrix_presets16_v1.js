/* Complex 16x16 operator presets and imaginary-coupling depth. */
autowatch = 1;
inlets = 4;
outlets = 4;
...
function pair(a,b,name) {
    zeroMatrices();
    for(var k=0;k<N;k++) re[k*N+k]=0.8;
    im[a*N+b]=0.2; im[b*N+a]=-0.2;
    source=name; emit();
}
...
function ring() {
    zeroMatrices();
    for(var k=0;k<N;k++) {
        var next=(k+1)%N; re[k*N+k]=0.65;
        im[k*N+next]=0.12; im[next*N+k]=-0.12;
    }
...
function imagdepth(v){ imagDepth=Math.max(0,Math.min(1,num(v,imagDepth))); emit(); }
