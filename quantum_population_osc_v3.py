...
from density.matrix_engine_4q import DensityMatrixEngine
...
def make_engine(seed: int) -> DensityMatrixEngine:
    ...
    def randomize_param(name, amount):
        if name in engine.params:
            value = engine.params[name]
            engine.set_param(
                name,
                value * rng.uniform(1.0 - amount, 1.0 + amount),
            )

    for q in range(4):
        randomize_param(f"x{q}", 0.10)
        randomize_param(f"y{q}", 0.10)
        randomize_param(f"z{q}", 0.10)

    randomize_param("coupling", 0.05)
    randomize_param("drift", 0.05)
    randomize_param("feedback", 0.05)
    randomize_param("drive", 0.05)

    randomize_param("dephase", 0.10)
    randomize_param("damping", 0.10)
    randomize_param("depolarize", 0.10)

    randomize_param("fb_x_neighbor_diff", 0.10)
    randomize_param("fb_y_neighbor_sum", 0.10)
    randomize_param("fb_z_neighbor_diff", 0.10)

    randomize_param("pair_coherence_gain", 0.10)
    randomize_param("pair_entropy_gain", 0.10)
    randomize_param("pair_xx_ratio", 0.10)
    randomize_param("pair_yy_ratio", 0.10)
    randomize_param("diagonal_pair_ratio", 0.10)
