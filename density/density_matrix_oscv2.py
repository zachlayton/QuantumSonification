...
class DensityMatrixEngine:
    ...
    def reset_state(self):
        ...
        self.rho = state_to_density(psi_00)

        for key in self.drift_state:
            self.drift_state[key] = 0.0

        print("Density matrix reset to |00><00|")

    def update_drift(self, drift_amount):
        """
        Ornstein-Uhlenbeck-like bounded drift.
        This removes fixed periodic behavior without becoming
        an unstructured random-number stream.
        """
        decay = 0.985
    ...
    def hamiltonian(self, control_values):
        ...
        current_entropy = von_neumann_entropy(self.rho)
        current_coherence = self.mean_coherence(self.rho)
        current_mutual_info = mutual_information(self.rho)

        # Feedback creates state-dependent transformation.
        entropy_force = feedback * (current_entropy / 2.0)
        coherence_force = feedback * current_coherence
        relation_force = feedback * current_mutual_info

        x0 = control_values["x0"] + self.drift_state["x0"]
        y0 = control_values["y0"] + self.drift_state["y0"]
        z0 = control_values["z0"] + self.drift_state["z0"]

        x1 = control_values["x1"] + self.drift_state["x1"]
        y1 = control_values["y1"] + self.drift_state["y1"]
        z1 = control_values["z1"] + self.drift_state["z1"]

        coupling = (
            control_values["coupling"]
            + 0.45 * self.drift_state["coupling"]
            + 0.80 * coherence_force
            - 0.25 * entropy_force
            + 0.20 * relation_force
        )

        coupling = np.clip(coupling, -2.0, 2.0)

        # Local field terms + entangling interaction terms.
        h = (
            x0 * XI
            + y0 * YI
            + z0 * ZI
    ...
    def unitary_step(self, hamiltonian, dt):
        """
        U = exp(-i H dt)
        Uses Hermitian eigendecomposition instead of scipy.
        """
    ...
    def mean_coherence(self, rho):
        ...
        values = [np.abs(rho[i, j]) for i, j in pairs]
    ...
    def step(self, dt):
        ...
        h = self.hamiltonian(control_values)
        self.unitary_step(h, dt)
