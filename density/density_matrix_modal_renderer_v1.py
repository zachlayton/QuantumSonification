#!/usr/bin/env python3
"""
density_matrix_modal_renderer_v1.py
...
class RenderConfig:
    ...
    output_path: Path = Path("density_matrix_modal_render_v1.wav")
    seed: int = 11
...
def excitation_envelope(
    ...
    if source_descriptor is not None:
        source_gain = config.hydrogen_gain * (
            0.35
            + 0.45 * float(source_descriptor.probability)
            + 0.20 * float(source_descriptor.energy)
        )
        source_gain *= 0.75 + 0.25 * float(source_descriptor.coherence)
        source_phase = float(source_descriptor.phase)
        source_probability = float(source_descriptor.probability)
        source_gain = float(np.clip(source_gain, 0.0, 2.0))

    if config.excitation_mode == "continuous":
        return np.full(n_samples, source_gain, dtype=np.float64)

    times = t0 + np.arange(n_samples) / sample_rate

    if config.excitation_mode == "impulse_train":
        phase = (times * config.impulse_rate_hz + source_phase) % 1.0
        impulse = phase < (config.impulse_rate_hz / sample_rate)
        env = np.zeros(n_samples, dtype=np.float64)
        env[impulse] = source_gain
        return env

    if config.excitation_mode == "probabilistic":
        probability = modal_energy * config.probability_threshold * source_probability
        probability = np.clip(probability, 0.0001, 0.25)
...
def render_modal_frame(
    ...
    for mode_i, (freq, amp, decay, phase, pan) in enumerate(
        ...
        if np.allclose(excitation, excitation[0]):
            ring = np.full_like(local_t, excitation[0])
...
def parse_args() -> RenderConfig:
    ...
    parser.add_argument("--output", type=Path, default=Path("density_matrix_modal_render_v1.wav"))
