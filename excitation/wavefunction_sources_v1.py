...
class PotentialWavefunctionSource:
    ...
    def _potential(self, time: float) -> Array:
        ...
        elif self.potential_name == "morse":
            axis_index = int(params.get("axis", 0))
            dissociation_energy = float(params.get("dissociation_energy", 8.0))
            inverse_length = float(params.get("inverse_length", 0.8))
            equilibrium = float(params.get("equilibrium", 0.0))
            coordinate = self.mesh[axis_index]
            value = dissociation_energy * (
                1.0 - np.exp(-inverse_length * (coordinate - equilibrium))
            ) ** 2
            for index, axis in enumerate(self.mesh):
                if index != axis_index:
                    value = value + 0.5 * self.mass * self.omega[index] ** 2 * axis**2
...
def create_wavefunction_source(name: str, **kwargs: Any) -> WavefunctionSource:
    ...
    if name == "morse_potential":
        return PotentialWavefunctionSource(
            **_merged(
                {
                    "potential": "morse",
                    "center": 0.8,
                    ...
                    "potential_parameters": {
                        "dissociation_energy": 8.0,
                        "inverse_length": 0.8,
                        "equilibrium": 0.0,
                    },
                    "source_name": "morse_potential",
                },
                kwargs,
            )
        )
    ...
    if name == "aharonov_bohm_effect":
        from excitation.gauge_wavefunction_sources_v1 import AharonovBohmRingSource

        translated = dict(kwargs)
        if "grid_shape" in translated:
            translated["n_points"] = translated.pop("grid_shape")
        return AharonovBohmRingSource(**translated)
    if name == "landau_levels":
        from excitation.external_field_sources_v1 import LandauLevelSource

        return LandauLevelSource(**kwargs)
    if name in ("hydrogen_zeeman", "hydrogen_stark"):
        from excitation.external_field_sources_v1 import (
            HydrogenStarkSource,
            HydrogenZeemanSource,
        )

        source_type = HydrogenZeemanSource if name == "hydrogen_zeeman" else HydrogenStarkSource
        return source_type(**kwargs)
    if name in ("helium_variational_two_electron", "delta_function_shell"):
        ...
        source_type = (
            HeliumVariationalTwoElectronSource
            if name == "helium_variational_two_electron"
            else DeltaFunctionShellSource
        )
        translated = dict(kwargs)
        if "grid_shape" in translated:
            translated["n_points"] = translated.pop("grid_shape")
        return source_type(**translated)
