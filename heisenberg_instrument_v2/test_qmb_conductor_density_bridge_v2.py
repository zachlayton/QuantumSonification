...
from heisenberg_instrument_v2.qmb_conductor_density_bridge_v2 import (
    BridgeConfig,
    CELL_COUNT,
...
class BridgeTests(unittest.TestCase):
    ...
    def test_observation_change_recommits_without_waiting_for_clock(self) -> None:
        client = Client()
        bridge = ConductorDensityQMBBridge(
            client,
            BridgeConfig(
                timing_mode="quantum",
                temporal_distance=3.0,
                temporal_process="fixed",
            ),
        )
        bridge.set_smoothing_ms(0.0)
        grid = []
        active = {0: 0.5, 1: 0.5, 16: 0.5, 17: 0.5}
        for index in range(CELL_COUNT):
            grid.extend([active.get(index, 0.0), 0.0, 0.0])
        self.assertTrue(bridge.accept_grid(*grid, now=1.0))
        initial_frame = dict(client.messages[-8:])
        initial_messages = len(client.messages)
        self.assertTrue(bridge.set_observable_phase(0.25))
        self.assertEqual(len(client.messages), initial_messages + 8)
        self.assertEqual(client.messages[-1][1], 2)
        observed_frame = dict(client.messages[-8:])
        self.assertNotEqual(
            initial_frame["/qmw/heisenberg/active/matrix/real"],
            observed_frame["/qmw/heisenberg/active/matrix/real"],
        )
    ...
    def test_quantum_clock_commits_on_density_displacement_not_elapsed_time(self) -> None:
        client = Client()
        bridge = ConductorDensityQMBBridge(
            client,
            BridgeConfig(
        ...
        def basis_state(index: int) -> list[float]:
            return [
                item
                for cell in range(CELL_COUNT)
                for item in (1.0 if cell == index * 16 + index else 0.0, 0.0, 0.0)
            ]

        first = basis_state(0)
        moved = basis_state(1)
        self.assertTrue(bridge.accept_grid(*first, now=1.0))  # initialization
        self.assertFalse(bridge.accept_grid(*first, now=101.0))  # time alone is inert
        self.assertTrue(bridge.accept_grid(*moved, now=101.001))
        self.assertGreater(bridge.last_clock_reading.delta_bures, 0.1)
        self.assertGreater(bridge.last_clock_reading.pulses, 0)

    def test_clock_controls_and_rate_fallback(self) -> None:
        bridge = ConductorDensityQMBBridge(Client())
        bridge.set_temporal_distance(0.05)
        bridge.set_temporal_process("fixed")
        bridge.set_coherence_depth(0.75)
        self.assertAlmostEqual(bridge.temporal_clock.distance_per_pulse, 0.05)
        self.assertEqual(bridge.temporal_clock.mode, "fixed")
        self.assertAlmostEqual(bridge.temporal_clock.coherence_depth, 0.75)
        bridge.set_timing_mode("rate")
        self.assertEqual(bridge.timing_mode, "rate")
        with self.assertRaises(ValueError):
            bridge.set_timing_mode("metronome")
