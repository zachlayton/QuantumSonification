...
function phase_depth(value) {
    phaseDepth = clamp(value, 0.0, 1.0);
    outlet(2, "phase_depth", phaseDepth);
}
...
function publishRows(d, complex, magnitudes, phases, frequencies, pans) {
    ...
    for (var row = 0; row < d; row++) {
        ...
        var dominantPan = 0.0;
        var dominantOffDiagonalWeight = -1.0;
        var dominantOffDiagonalPhase = 0.0;
        for (var column = 0; column < d; column++) {
            ...
            if (row !== column && weight > dominantOffDiagonalWeight) {
                dominantOffDiagonalWeight = weight;
                dominantOffDiagonalPhase = phases[index];
            }
        ...
        // Observable phase rotates off-diagonal relationships while leaving
        // their magnitudes nearly invariant. Use the strongest such relation
        // as a circular control signal so the rotation remains audible even at
        // the component end of the mapping crossfade. Each Rings parameter is
        // approached with a different quadrature and depth; line~ in the voice
        // patch smooths the resulting motion.
        var observedPhase = dominantOffDiagonalWeight > 1.0e-12 ?
            dominantOffDiagonalPhase : meanPhase;
        var observedSin = 0.5 + 0.5 * Math.sin(observedPhase);
        var observedCos = 0.5 + 0.5 * Math.cos(observedPhase);
        structure = mix(structure, 0.08 + 0.84 * observedCos, 0.32 * phaseDepth);
        brightness = mix(brightness, 0.08 + 0.90 * observedSin, 0.58 * phaseDepth);
        damping = mix(damping, 0.20 + 0.72 * (1.0 - observedSin), 0.34 * phaseDepth);
        position = mix(position, observedSin, 0.52 * phaseDepth);
        var featureValues = {
        ...
        var phaseTurn = (observedPhase + Math.PI) / (2.0 * Math.PI);
