...
autowatch = 1;
inlets = 1;
outlets = 7; // status, buffer bangs, waveform preview, Pauli preview, local XYZ, IBM XYZ
...
function msg_float(value) { xfade(value); }
function xfade(value) {
    sourceMix = clamp(Number(value), 0, 1);
    renderPreview();
}

function renderPreview() {
    var localWave = states.local && states.local.rows ? states.local.rows[3] : null;
    var ibmWave = states.ibm && states.ibm.rows ? states.ibm.rows[3] : null;
    if (localWave || ibmWave) {
        var preview = [];
        for (var i = 0; i < 256; i++) {
            if (!localWave) preview.push(ibmWave[i]);
            else if (!ibmWave) preview.push(localWave[i]);
            else preview.push((1-sourceMix)*localWave[i] + sourceMix*ibmWave[i]);
        }
        outlet(3, preview);
    }

    var localPauli = states.local ? states.local.c : null;
    var ibmPauli = states.ibm ? states.ibm.c : null;
    if (localPauli || ibmPauli) {
        var coefficients = [];
        for (var term = 0; term < 15; term++) {
            if (!localPauli) coefficients.push(ibmPauli[term]);
            else if (!ibmPauli) coefficients.push(localPauli[term]);
            else coefficients.push(
                (1-sourceMix)*localPauli[term] + sourceMix*ibmPauli[term]
            );
        }
...
function irsize(value) {
    irSize = clamp(Number(value), 0.1, 1.0);
    for (var source in states) {
        var state = states[source];
        if (!state || !state.waveform) continue;
        renderIR(source, state, state.waveform);
        outlet(source === "local" ? 1 : 2, "bang");
    }
    outlet(0, ["ir_size", irSize]);
}
function irscale(value) {
    irScale = clamp(Number(value), 1.0, 8.0);
    for (var source in states) {
        var state = states[source];
        if (!state || !state.waveform) continue;
        renderIR(source, state, state.waveform);
...
function renderIR(source, state, waveform) {
    ...
    var rt60Samples=Math.round(length*irSize*(0.35+0.60*entropy)), peak=0, energy=0;
    ...
    var reflectionA=Math.round(length*irSize*0.017);
    var reflectionB=Math.round(length*irSize*0.043);
    var reflectionC=Math.round(length*irSize*0.091);
    ...
    var scale=Math.min(energyScale,peakScale)*irScale;
    // Drive the normalized IR without ever allowing a sample above -0.45 dBFS.
    if (peak*scale > 0.95) scale=0.95/peak;
    for (var j=0;j<length;j++) ir[j]*=scale;
    var target=new Buffer(source === "local" ? "qmw_v5_ir_local" : "qmw_v5_ir_ibm");
...
function render(source, updateIR) {
    ...
    var target=new Buffer(source === "local" ? "qmw_v5_local" : "qmw_v5_ibm");
    var storedRows=[];
    for (var r=0;r<4;r++) {
        var scaled=[]; for (var s=0;s<256;s++) scaled.push(rows[r][s]*scale);
        storedRows.push(scaled);
...
function renderDifference() {
    var local=states.local, ibm=states.ibm;
    if (!local || !ibm || !local.rows || !ibm.rows) return;
    var target=new Buffer("qmw_v5_difference"), rms=0, count=0, peak=0;
    for (var row=0;row<4;row++) {
        var delta=[];
        for (var i=0;i<256;i++) {
            // Fixed 0.5 scale preserves absolute error magnitude while bounding
            // the difference of two full-scale buffers to approximately unity.
            var value=0.5*(ibm.rows[row][i]-local.rows[row][i]);
            delta.push(value); rms+=value*value; count++;
            peak=Math.max(peak,Math.abs(value));
        }
        target.poke(1,row*256,delta);
    }
    outlet(0,["difference_ready",local.revision,ibm.revision,
        Math.sqrt(rms/count),peak]);
}
