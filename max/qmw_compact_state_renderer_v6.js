...
function renderIR(source, state, waveform) {
    ...
    var target=new Buffer(source === "local" ? "qmw_v6_ir_local" : "qmw_v6_ir_ibm");
...
function render(source, updateIR) {
    ...
    var target=new Buffer(source === "local" ? "qmw_v6_local" : "qmw_v6_ibm");
    var storedRows=[];
    for (var r=0;r<4;r++) {
...
function renderDifference() {
    ...
    var target=new Buffer("qmw_v6_difference"), rms=0, count=0, peak=0;
    for (var row=0;row<4;row++) {
        var delta=[];
        for (var i=0;i<256;i++) {
