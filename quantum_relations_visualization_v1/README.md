...
python quantum_relations_visualization_v1/quantum_relations_osc_v1.py \
  output/current_density_matrix.npz \
  --rho-key rho \
  --out-port 7450 \
  --control-port 7451 \
  --state-control-port 7403 \
  --basis Z \
...
These controls and CLI flags are backward compatible. In `collapse` mode the
relations process no longer edits its private density-matrix copy. It sends
`/quantum/state/control/measure` to the authoritative four-qubit engine on UDP
7403. The engine samples the outcome, applies the projector to canonical
`self.rho`, advances the revision, and returns a result to UDP 7451. Relations
then republishes the existing `/quantum/measurement/*` visualization messages.
The response also carries the revisioned canonical density matrix, allowing
relations to recompute its metrics immediately from the engine-owned state.
After this authoritative stream is observed, automatic file watching is
suppressed so a later stale file cannot replace the displayed state. File
watching remains the unchanged fallback when the density engine is absent.

`probe` follows the same request path but leaves the canonical state and its
revision unchanged. If the density engine is not running, a measurement stays
pending; the relations process never falls back to a private collapse.
