...
For a portable install, copy this entire package folder into `Documents/Max 8/Packages` (or the corresponding Max 9 Packages folder), restart Max, and open the main patch from the package's `patchers` directory. The bundled viewer, IR files, Gen sources and universal HIRT `multiconvolve~` external travel with the package.
