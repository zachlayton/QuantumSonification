...
def test_living_osc_publishes_complete_revision(tmp_path: Path) -> None:
    ...
    with LivingSpectralGeometry(
        ...
        assert message_map["/living/spectral/history/shape"] == [
            1,
            state.mode_count,
        ]
        assert (
            message_map["/living/spectral/history/phase_domain"]
            == "complex_eigenfield_mode"
        )
        assert message_map["/living/spectral/playback/blur_width"] == 1
        publisher.handle_spectral_blur_width(
            "/living/spectral/control/blur_width", 6
        )
        publisher.handle_spectral_transition(
            "/living/spectral/control/transition", 0.35
        )
        publisher.handle_spectral_freeze(
            "/living/spectral/control/freeze", 1
        )
        assert publisher.spectral_playback.blur_width == 6
        assert publisher.spectral_playback.transition == 0.35
        assert publisher.spectral_playback.frozen
        message_map = dict(client.messages)
        assert message_map["/living/spectral/playback/blur_width"] == 6
        assert message_map["/living/spectral/playback/transition"] == 0.35
        assert message_map["/living/spectral/playback/freeze"] == 1
