...
from quantum_oscilloscope.quantum_oscilloscope_v1 import (
    QuantumOscilloscope,
...
class MLXQuantumEngine:
    ...
    def send_density_matrix_to_processing(self, rho_np):
        ...
        source = self.density_engine.excitation_source

        if source is not None and hasattr(source, "probability_distribution"):
            probability = source.probability_distribution
            print(
                "WAVEPACKET SEND:",
    ...
    def step(self):
        ...
        circuit_entropy = circuit_state.get(
            "single_qubit_entropy",
            {},
        )

        circuit_pilot = circuit_state.get("pilot", None)
        ...
        self.osc.send_message(
            "/qmw/circuit/q0_entropy",
            float(circuit_entropy.get(0, 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/q1_entropy",
            float(circuit_entropy.get(1, 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/q2_entropy",
            float(circuit_entropy.get(2, 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/q3_entropy",
            float(circuit_entropy.get(3, 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/entropy_spread",
            float(circuit_state.get("entropy_spread", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/entropy_centroid",
            float(circuit_state.get("entropy_centroid", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/entropy_centroid_norm",
            float(circuit_state.get("entropy_centroid_norm", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/entropy_motion",
            float(circuit_state.get("entropy_motion", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/entropy_motion_mean",
            float(circuit_state.get("entropy_motion_mean", 0.0)),
        )
        self.osc.send_message(
        ...
        self.osc.send_message(
            "/qmw/circuit/coherence",
            float(circuit_state.get("coherence", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/purity",
            float(circuit_state.get("purity", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/entropy",
            float(circuit_state.get("entropy", 0.0)),
        )

        self.osc.send_message(
            "/qmw/circuit/column",
            int(circuit_state.get("column", 0)),
        )

        self.osc.send_message(
            "/qmw/circuit/pilot_node",
            int(getattr(circuit_pilot, "node", 0))
        )
        self.osc.send_message(
            "/qmw/circuit/pilot_velocity",
            float(getattr(circuit_pilot, "velocity", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/phase_gradient",
            float(getattr(circuit_pilot, "phase_gradient", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/branch_entropy",
            float(getattr(circuit_pilot, "branch_entropy", 0.0)),
        )

        self.osc.send_message(
            "/qmw/circuit/gate_applied",
            int(circuit_state.get("gate_applied", 0)),
        )

        self.osc.send_message(
            "/qmw/circuit/delta_rho",
            float(circuit_state.get("gate_delta_rho", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/last_column",
            int(circuit_state.get("last_gate_column", -1)),
        )
        self.osc.send_message(
        ...
        self.osc.send_message(
            "/qmw/circuit/gate_energy",
            float(circuit_state.get("gate_energy", 0.0)),
        )
        self.osc.send_message(
            "/qmw/circuit/event_count",
            int(circuit_state.get("gate_event_count", 0)),
        )

        self.osc.send_message(
            "/qmw/density/delta_rho/max",
            self.density_delta_rho_max,
        )
        self.osc.send_message(
            "/qmw/density/delta_rho/mean",
            self.density_delta_rho_mean,
        )
        ...
        self.osc.send_message(
            "/qmw/entanglement/entropy_spread",
            metrics.get("entropy_spread", 0.0),
        )
        self.osc.send_message(
            "/qmw/entanglement/entropy_centroid",
            metrics.get("entropy_centroid", 0.0),
        )
        self.osc.send_message(
            "/qmw/entanglement/entropy_centroid_norm",
            metrics.get("entropy_centroid_norm", 0.0),
        )
        self.osc.send_message(
            "/qmw/entanglement/entropy_motion",
            metrics.get("entropy_motion", 0.0),
        )
        self.osc.send_message(
            "/qmw/entanglement/entropy_motion_mean",
            metrics.get("entropy_motion_mean", 0.0),
        )
        self.osc.send_message(
        ...
        self.osc.send_message(
            ...
            int(metrics.get("entropy_dominant_qubit", 0)),
        )
        ...
        if source is not None and hasattr(source, "wavefunction"):
            ...
            self.visual_osc.send_message(
                ...
                [float(v) for v in np.real(psi)],
            )

            self.visual_osc.send_message(
                "/qmw/source/wavepacket/imag",
                [float(v) for v in np.imag(psi)],
            )

            self.visual_osc.send_message(
                "/qmw/source/wavepacket/phase",
                [float(v) for v in np.angle(psi)],
            )
