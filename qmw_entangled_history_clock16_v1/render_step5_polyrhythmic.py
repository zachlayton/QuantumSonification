#!/usr/bin/env python3
"""Render the sixteen-voice polyrhythmic history proof."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-directory",
        type=Path,
        default=Path("output/entangled_history_clock16_v1/step5_polyrhythmic"),
    )
    ...
    parser.add_argument("--interval-ms-end", type=float, default=112.0)
    ...
    config = PolyrhythmicHistoryConfig(
        sample_rate=int(args.sample_rate),
        duration_seconds=float(args.duration),
        rhythm_word=str(args.rhythm_word),
        reference_interval_ms_start=float(args.interval_ms),
        reference_interval_ms_end=float(args.interval_ms_end),
    )
    render = render_polyrhythmic_history(history, config)
    paths = write_polyrhythmic_history(
        render,
