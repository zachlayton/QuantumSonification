...
an emergent geometry through a positivity-preserving quantum channel.
...
Instead it constructs a geometry-conditioned isometry V between them:

    quantum state rho
        -> geometry-conditioned quantum channel
        -> modal density matrix Gamma
...
class QuantumEigenfieldConfig:
    ...
    coupling_mode: str = "quantum_channel"
...
@dataclass(frozen=True)
class ModalQuantumState:
    """Density-operator state produced in the geometric modal basis."""
...
class QuantumGeometryChannel:
    """Geometry-conditioned CPTP map from a quantum state to modal space.

    If the modal dimension is at least the quantum dimension, the environment
    has dimension one and the map is exactly ``Gamma = V rho V^dagger``.  For
    fewer modes, the same construction embeds into modes x environment and
    traces out the environment, avoiding non-trace-preserving truncation.
    """

    def __init__(
        self,
        geometric_eigenvalues: np.ndarray,
        ...
        *,
        ...
        self.eigenvalues = np.asarray(
            geometric_eigenvalues, dtype=np.float64
        ).reshape(-1)
        self.eigenvectors = np.asarray(
            geometric_eigenvectors, dtype=np.complex128
        )
        self.quantum_dimension = int(quantum_dimension)
        self.seed = int(seed)
        if self.quantum_dimension < 1:
            raise ValueError("quantum_dimension must be positive")
        if self.eigenvectors.ndim != 2:
            raise ValueError("geometric eigenvectors must be a matrix")
        if self.eigenvectors.shape[1] != len(self.eigenvalues):
        ...
        self.environment_dimension = int(
            np.ceil(self.quantum_dimension / len(self.eigenvalues))
        )
        self.isometry: np.ndarray | None = None

    def _mode_signatures(self) -> np.ndarray:
        values = self.eigenvalues
        value_scale = max(float(np.max(np.abs(values))), 1e-15)
        lambda_signature = values / value_scale
        weights = np.abs(self.eigenvectors) ** 2
        weights /= np.maximum(np.sum(weights, axis=0, keepdims=True), 1e-15)
        coordinate = np.linspace(-1.0, 1.0, self.eigenvectors.shape[0])
        centroid = coordinate @ weights
        participation = 1.0 / np.maximum(np.sum(weights**2, axis=0), 1e-15)
        participation /= max(float(np.max(participation)), 1e-15)
        return lambda_signature + 0.37 * centroid + 0.19 * participation

    def build_isometry(self) -> np.ndarray:
        mode_count = len(self.eigenvalues)
        environment = self.environment_dimension
        row_count = mode_count * environment
        dimension = self.quantum_dimension
        signatures = self._mode_signatures()
        rows = np.arange(row_count, dtype=np.float64)[:, None]
        columns = np.arange(dimension, dtype=np.float64)[None, :]
        modal_rows = (np.arange(row_count) // environment).astype(np.int64)
        geometry = signatures[modal_rows][:, None]
        seed_phase = (self.seed % 104729) / 104729.0
        phase = 2.0 * np.pi * (
            (rows + 0.5) * (columns + 0.5) / (row_count + dimension)
            + geometry * (columns + 1.0) / (dimension + 1.0)
            + seed_phase * (rows + columns + 1.0)
        )
        amplitude = 1.0 + 0.23 * np.cos(
            geometry * (columns + 1.0) + 0.31 * (rows + 1.0)
        )
        raw = amplitude * np.exp(1j * phase)
        ...
        if error > 1e-10:
            raise ValueError(f"channel isometry error is {error:.3e}")
        self.isometry = isometry
        return isometry

    def project(self, rho: np.ndarray) -> ModalQuantumState:
        state = validate_density_matrix(rho)
        if state.shape[0] != self.quantum_dimension:
            raise ValueError("density-matrix dimension does not match channel")
        isometry = self.build_isometry() if self.isometry is None else self.isometry
        lifted = isometry @ state @ isometry.conj().T
        mode_count = len(self.eigenvalues)
        environment = self.environment_dimension
        tensor = lifted.reshape(
            mode_count, environment, mode_count, environment
        )
        modal_state = np.einsum("aebe->ab", tensor, optimize=True)
        modal_state = validate_density_matrix(modal_state, atol=1e-8)
        probabilities = np.real(np.diag(modal_state)).copy()
        coherences = modal_state - np.diag(probabilities)
        return ModalQuantumState(
            density_matrix=modal_state,
            probabilities=probabilities,
            coherences=coherences,
            channel_isometry=isometry,
...
class QuantumEigenfieldEngine:
    ...
    def __init__(
        ...
        self.quantum_geometry_channel: QuantumGeometryChannel | None = None
        self.modal_quantum_state: ModalQuantumState | None = None
        self.modal_density_matrix: np.ndarray | None = None
    ...
    def build_coupling_matrix(self) -> np.ndarray:
        ...
        quantum_channel:
            Geometry-conditioned isometry for a positivity-preserving channel.

        legacy_hybrid / hybrid:
        ...
        """
        if self.config.coupling_mode == "quantum_channel":
            channel = QuantumGeometryChannel(
                self.geometric_eigenvalues,
                self.geometric_eigenvectors,
                self.rho.shape[0],
                seed=self.config.coupling_seed,
            )
            self.quantum_geometry_channel = channel
            self.coupling_matrix = channel.build_isometry()
        ...
        legacy_mode = self.config.coupling_mode
        if legacy_mode == "legacy_artistic":
            legacy_mode = "hybrid"
        elif legacy_mode.startswith("legacy_"):
            legacy_mode = legacy_mode.removeprefix("legacy_")

        if legacy_mode == "spectral":
            ...
            raise ValueError(
                "coupling_mode must be quantum_channel or a legacy mode: "
                "legacy_artistic, legacy_hybrid, legacy_spectral, legacy_pauli"
            )
    ...
    def project(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self.config.coupling_mode == "quantum_channel":
            assert self.quantum_geometry_channel is not None
            modal_state = self.quantum_geometry_channel.project(self.rho)
            probabilities = modal_state.probabilities.copy()
            values, vectors = np.linalg.eigh(modal_state.density_matrix)
            reference = vectors[:, int(np.argmax(values))]
            phases = np.ones(len(reference), dtype=np.complex128)
            visible = np.abs(reference) > 1e-14
            phases[visible] = reference[visible] / np.abs(reference[visible])
            complex_amplitudes = np.sqrt(probabilities) * phases
            self.modal_quantum_state = modal_state
            self.modal_density_matrix = modal_state.density_matrix
        ...
        self.mode_probabilities = probabilities
        self.mode_amplitudes = complex_amplitudes
        self.modal_density_matrix = np.diag(probabilities).astype(
            np.complex128
        )
        return probabilities, complex_amplitudes
    ...
    def mode_descriptors(self) -> dict[str, Any]:
        ...
        return {
            ...
            "modal_density_trace": (
                None
                if self.modal_density_matrix is None
                else float(np.real(np.trace(self.modal_density_matrix)))
            ),
            "modal_density_min_eigenvalue": (
                None
                if self.modal_density_matrix is None
                else float(
                    np.min(np.linalg.eigvalsh(self.modal_density_matrix)).real
                )
            ),
            "channel_environment_dimension": (
                None
                if self.modal_quantum_state is None
                else self.modal_quantum_state.environment_dimension
            ),
            "field_mean": (
    ...
    def export(self, prefix: str | Path) -> dict[str, Path]:
        ...
        assert self.mode_probabilities is not None
        assert self.mode_amplitudes is not None
        assert self.modal_density_matrix is not None
        assert self.spatial_eigenfield is not None
        ...
        state: dict[str, Any] = {
            ...
            "modal_density_matrix": self.modal_density_matrix,
            "modal_coherences": (
                self.modal_density_matrix
                - np.diag(np.diag(self.modal_density_matrix))
            ),
        ...
        optional_state = {
            "channel_isometry": (
                None
                if self.modal_quantum_state is None
                else self.modal_quantum_state.channel_isometry
            ),
            "channel_environment_dimension": (
                None
                if self.modal_quantum_state is None
                else np.asarray(
                    self.modal_quantum_state.environment_dimension,
                    dtype=np.int64,
                )
            ),
