...
# This resonator-specific copy lives one directory below the canonical QMW
# package tree (analysis/, density/, field/, hamiltonian/, ...).  Make it
# runnable from any working directory without requiring a manual PYTHONPATH.
...
class MLXQuantumResonatorEngine:
    def __init__(
        ...
        if not audio_path.is_file():
            raise FileNotFoundError(
                f"QMW analysis audio file does not exist: {audio_path}. "
                "Choose an existing file with --audio-path."
            )
    ...
    def step(self):
        ...
        if self.density_field_elapsed >= self.density_field_interval:
            ...
            self.send_density_field_frame(density_scan, metrics)
    ...
    def send_density_field_frame(self, density_scan, metrics):
