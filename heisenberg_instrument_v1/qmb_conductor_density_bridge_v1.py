...
Rings instrument continues listening on UDP 7400.  The conductor may publish at
30 Hz; the bridge deliberately commits at a slower musical rate.
"""
...
class ConductorDensityQMBBridge:
    ...
    def set_observable_phase(self, normalized: object) -> None:
        value = max(0.0, min(1.0, float(normalized)))
        with self.lock:
            self.observable_phase_radians = math.tau * value

    def set_observable_phase_radians(self, radians: object) -> None:
        with self.lock:
            self.observable_phase_radians = float(radians) % math.tau

    def set_observable_phase_offset(self, qubit: int, normalized: object) -> None:
        value = max(0.0, min(1.0, float(normalized)))
        with self.lock:
            self.observable_phase_offsets[int(qubit)] = math.tau * value

    def set_observable_enabled(self, enabled: object) -> None:
        with self.lock:
            self.observable_enabled = bool(int(float(enabled)))

    def set_observable(self, name: object) -> None:
        ...
        with self.lock:
            if selected == "FOUR":
                ...
                self.observable_tilt_radians = math.pi / 4.0
                return
            if selected not in {"X0", "Y0", "Z0", "XZ0"}:
    ...
    def publish_if_due(self, now: float) -> bool:
        interval = 1.0 / self.output_hz
        if self.real is None or self.imag is None or now - self.last_publish_time < interval:
            return False
...
def main() -> int:
    ...
    bridge = ConductorDensityQMBBridge(
        ...
        BridgeConfig(output_hz=args.output_hz),
    )
